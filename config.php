<?php
/**
 * Configuration globale de Symphony
 * Tous les paramètres de l'application sont centralisés ici
 */
...
class Config {
    ...
    // Environnement
    const ENV = 'development'; // development, production, testing
    ...
    // Fournisseur (NestCorporation) - bootstrap simple
    const PROVIDER_BOOTSTRAP_ENABLED = true;
    const PROVIDER_ADMIN_EMAIL = 'provider@nestcorporation.com';
    const PROVIDER_ADMIN_PASSWORD = 'ChangeThisPassword123!';
    const PROVIDER_ADMIN_NAME = 'Nest Corporation';
    const PROVIDER_API_KEY_NAME = 'Default Nest API Key';
    const PROVIDER_API_KEY = 'nest_dev_key_change_this';
    const PROVIDER_WEBHOOK_SECRET = 'nest_webhook_secret_change_this';
    ...
    const CRON_TOKEN = 'change_this_cron_token';
    
    // Sécurité
    const SESSION_NAME = 'symphony_session';
    const SESSION_LIFETIME = 7200; // 2 heures
    const CSRF_TOKEN_NAME = 'csrf_token';
    const PASSWORD_COST = 12; // Bcrypt cost
    ...
    // TVA par défaut (RDC)
    const DEFAULT_VAT_RATE = 16; // 16% en RDC
    ...
    const AI_SURVEILLANCE_INTERVAL = 300; // 5 minutes en secondes
    const AI_MAX_MEMORY_SUMMARY = 500; // caractères max par résumé mémoire
    const AI_DEFAULT_PROVIDER = 'gemini';
    const AI_PROVIDERS = [
        'gemini' => [
            'api_key' => 'AIzaSyDW3ybrvueyhzTb8TDgLnxd4opZ_QPTyx0',
    ...
    public static function init() {
        ...
        // Journalisation globale des erreurs applicatives
        if (class_exists('\\App\\Core\\AppLogger')) {
            ...
            set_exception_handler(static function (\Throwable $exception): void {
                ...
                if (Config::isDev()) {
                    echo 'Erreur interne: ' . htmlspecialchars($exception->getMessage(), ENT_QUOTES, 'UTF-8');
                } else {
